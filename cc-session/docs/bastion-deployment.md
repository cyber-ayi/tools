...
work happens on your dev machine (mbp) over SSH.
...
- mbp stays the single source of truth for workspace files (no Syncthing
  cloning, no two-machine state divergence)
- A clean DR fallback (mbp's own cc-session) when the VPS is unreachable

For the alternative "all on mbp" deployment, just run `cc-session -d` on
mbp and skip this doc — but you lose phone-access-while-asleep.
...
Tailscale-IP restriction into mbp, and an Anthropic OAuth token for RC.
...
`~/cc/.env` (the actual secret values) **stays on mbp**.

## Prerequisites

- Ubuntu / Debian VPS reachable via Tailscale (this guide assumes Servarica MTL)
- mbp running macOS, on the same tailnet
...
## Step 2 — Generate VPS→mbp SSH bridge key
...
mbp, including the `from="<vps-tailscale-ip>"` IP restriction.

Switch to mbp (or another existing SSH session into mbp), append the
line:

```bash
ssh me@mbp.tail4a8253.ts.net
...
```bash
ssh -i ~/.ssh/cc_bridge_ed25519 me@mbp.tail4a8253.ts.net 'echo ok && hostname'
```

Should print `ok` + your mbp's hostname.
...
## Step 4 — Install LaunchAgent on mbp (optional DR fallback)

Only useful if you want mbp to spawn its own cc-session at user login,
...
```bash
cp ~/tools/cc-session/install/launchd-mbp.plist.template \
...
Note: this will spawn a **separate** RC URL on mbp, distinct from the
VPS's URL. Bookmark both in your phone:

- `vps-url` → primary, always-on
- `mbp-url` → fallback, valid only when mbp is awake
...
3. Click in, send a prompt that needs mbp data, e.g.
   `What's the most recent commit message in ~/cc?`
4. claude on the VPS uses the Bash tool with `ssh me@mbp.tail4a8253.ts.net 'cd ~/cc && git log -1 --format=%s'`
5. The reply should reflect mbp's actual commit log
...
ssh me@mbp      'cc-session --status'         # mbp fallback check
```
...
ssh me@mbp       'tail -f /tmp/cc-session.{out,err}.log' # mbp launchd
```
...
| VPS-claude can't SSH to mbp | mbp Tailscale IP changed, or VPS Tailscale IP changed (breaks the `from=` clause) | re-run `setup-bridge-key.sh`, update mbp's `authorized_keys` |
| mbp asleep, VPS RC works but file ops fail | by design — wake mbp via `wake on lan` or external power | rare; in practice mbp wakes from Tailscale incoming traffic |
