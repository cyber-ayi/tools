...
`--adopt` is idempotent: if RC is already active, it just prints the
existing URL by reading the pane scrollback (no `/remote-control`
keystroke is sent, so it won't open the management menu).
